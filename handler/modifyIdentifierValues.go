...
func ModifyIdentifierValues(identifier string, identifierValues model.ValueListModule, protocolType string) (res interface{}, err error) {
	modifyIdentifierValues := model.MsgCreateID{
	...
	var body = modifyIdentifierValues.CreateToBytes()
	// 修改标识值消息头结构
	msgHeader := model.MessageHeader{
		OpCode:               u.OC_MODIFY_VALUE,
		ResponseCode:         0,
